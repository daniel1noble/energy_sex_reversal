...
Bass_loo <- as.data.frame(loo_compare(Bas_m1_brms, Bas_m2_brms))
...
# SD prediction data and anova comparison
...
```
<p>*Bassiana duperreyi –* According to loo values there was equal support for both models, but preference for the second model that accounted for heteroscedasticity ([heteroscedastic model  – homoscedastic model] loo: ELPD = -5.6, SE=6.8). Determining if sex-reversed individuals (XXm) supported the like phenotype or like genotype hypothesis when comparing overall metabolic rate, there was minimal overlap in slope differences in overall metabolic rate between XXm and concordant females (XXf). Whereas there was a high degree of overlap in slope differences in overall metabolic rates between concordant males (XYm) and XXm, but we did not detect any significant differences between sex

(Table 1). When accounting for how metabolic rate varied across body mass and sex, there was evidence for the like phenotype hypothesis. As mass increased the relationship in predicted metabolic rate for XXm and XYm were more similar and lower than XYf (Fig. 2A). When comparing prediction values derived from the Brms models, the distribution of predicted metabolic at three areas (mean, +1SD, and -1SD) of log body mass varied significantly when comparing each sex.<p>
...
hp<- hypothesis(Pog_m2_brms, "zlogMass + sexZZf:zlogMass = 0")
plot(hp)
# contrast genotype slopes 
pog.genotype <- as.mcmc(ZZf - ZZm)
mean(pog.genotype)
HPDinterval(pog.genotype
hg <- hypothesis(Pog_m2_brms, "sexZZf:zlogMass + sexZZm:zlogMass = 0")
plot(hg)
