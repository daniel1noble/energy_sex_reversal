...
```{body size, loo comparison, echo=FALSE, message=FALSE, warning=FALSE}
# raw data
...
Bass_loo <- loo_compare(Bas_m1_brms, Bas_m2_brms)
...
```
# Results
<p>*Bassiana duperreyi –* According to loo values there was equal support for both models, but preference for the second model that accounted for heteroscedasticity ([heteroscedastic model  – homoscedastic model] loo: ELPD = -5.2, SE=6.9). Determining if sex-reversed individuals (XXm) supported the like phenotype or like genotype hypothesis when comparing overall metabolic rate, there was minimal overlap in slope differences in overall metabolic rate between XXm and concordant females (XXf). Whereas there was a high degree of overlap in slope differences in overall metabolic rates between concordant males (XYm) and XXm, suggesting support for like phenotype hypothesis for sex-reversed individuals (Fig. 2A; Table 1). When accounting for how metabolic rate varied across body mass and sex, there was further evidence for the like phenotype hypothesis. As mass increased the relationship in predicted metabolic rate for XXm and XYm were more similar and lower than XYf (Fig. 3A). There were no differences in body mass across sex class (p>0.05; Fig. 3A).<p>
```{body size, loo comparison, echo=FALSE, message=FALSE, warning=FALSE}
# raw data
...
Pog_loo <- loo_compare(Pog_m1_brms, Pog_m2_brms)
...
pMCMC <- 1 - (table(bm_diff$samples$H1 > 0)[1] / (length(bm_diff$samples$H1) - 1))      
```
<p>*Pogona vitticeps -* According to loo values there was preference for the second model that accounted for heteroscedasticity ([heteroscedastic model  – homoscedastic model] loo: ELPD = XX, XX). Determining if sex-reversed individuals (ZZf) supported the like phenotype or like genotype hypothesis when comparing overall metabolic rate, we found minimal overlap in slope differences in metabolic rate between ZZf and concordant males (ZZm). Whereas there was a high degree of overlap in slope differences in metabolic rate between concordant females (ZWf) and ZZf (Fig. 2B; Table 1), which indicated support for like phenotype hypothesis for sex-reversed individuals. These results were further supported when accounting for the relationship between body mass and sex. As mass increased, the predicted metabolic rate for ZWf and ZZf was more similar and higher than ZZm (Fig. 3B). We found no differences in body mass across sex class (F2, 131= 0.37, p>0.05; Figure 3B). <p>