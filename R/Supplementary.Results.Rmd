...
knitr::include_graphics("./Final.Figures/FigS1")
```
Figure S1. 
\newpage
# Tables & Figures
```{r Fig1,echo=FALSE, results='asis'}
knitr::include_graphics("./Final.Figures/FigS2")
```
...
### 
# final table
tables4 <- rbind(Bass.mass.tbl, Bass.svl.tbl) 
...
# 2) MASS
Pog.Mass.mod <- readRDS(file ="~/Dropbox/energy_sex_reversal/models/Pog_growth_metabolism")
...
\newpage
Table S6. BRMS Model coefficients testing for the relationship between mean metabolic rate and mass growth rate estimates across sex class for *Bassiana duperryii* and *Pogona vitticeps*. Mass growth rate for *B. duperryii* is expressed in cg/d and mass growth rate for *P. vitticeps* is expressed in g/d.
```{r TableS6, echo=FALSE}
###
# Pogona growth and matabolism
# 1) MASS
Pog.Mass.mod <- readRDS(file ="~/Dropbox/energy_sex_reversal/models/Pog_growth_metabolism")
Pog.mass.tbl <- lazerhawk::brms_SummaryTable(Pog.Mass.mod)
Pog.mass.tbl[2,1] <- "log(mean_O2)"
Pog.mass.tbl <- Pog.mass.tbl %>% mutate("Species" = "P. vitticeps") %>% dplyr::select(-c("Est.Error"))

# bassiana growth and metabolism
#2) MASS
Bass.Mass.mod <- readRDS(file = "~/Dropbox/energy_sex_reversal/models/Bass_growth_metabolism")
Bass.mass.tbl <- lazerhawk::brms_SummaryTable(Bass.Mass.mod)
Bass.mass.tbl[2,1] <- "log(mean_O2)"
Bass.mass.tbl <- Bass.mass.tbl %>% mutate("Species" = "B. duperryii") %>% dplyr::select(-c("Est.Error"))


### 
# final table
tables6 <- rbind(Bass.mass.tbl, Pog.mass.tbl)
Tables6 <- tables6[,c(5,1,2,3,4)]
Tbl.s6 <- flextable(Tables6) %>% 
  italic(i = 1:8, j = 1, italic = TRUE) %>% 
  merge_v(j = c("Species")) %>%  
  hline(i=c(4), j = 1:4, part="body") %>% 
  autofit(part = "all") %>% 
  align(align ="center", part = "all") %>% 
  font(part = "all", fontname = "Times New Roman") %>% 
  fix_border_issues() %>% 
