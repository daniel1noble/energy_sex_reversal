############################################################
# Code for processing BASSIANA SEX REVERSAL 21 MAY 2021
...
## Find files
files <- paste0("./data/bassiana/", list.files("./data/bassiana/"))
...
plotting <- function(data, files, channel, col = "red", method = "spline", ...){
  ...
  quartz.save(file = paste0(gsub("./data/bassiana/", "./figures/bassiana_resp_figures/", files), "_", channel, ".pdf"), type = "pdf")
...
datCO2$.id <- gsub("./data/bassiana/", "", datCO2$.id)
datCO2$match <-  as.character(interaction(datCO2$.id, datCO2$marker))

### O2 DATA
# subset from data file name
datO2$.id <- gsub("./data/bassiana/", "", datO2$.id)
datO2$match <-  as.character(interaction(datO2$.id, datO2$marker))

### METADATA 
# Bring in the meta-data file with weight etc
meta_data <- read.csv("final.bassiana.metadata/bassiana.metabolic.sexreversal.csv", stringsAsFactors = FALSE)
# Bring in genotype meta-datafile
geno_data <- read.csv("./final.bassiana.metadata/Genotype.dat.csv", stringsAsFactors = FALSE)
geno_data$bd_liz_id <- geno_data$KW_ID
# merge metadata files by lizard ID
dat <- merge(meta_data, geno_data, by ="bd_liz_id" )
...
write.csv(O2_BMR, file = "final.analysis.data/Bassiana.finalO2.sexreversal.analysis.data.clean.csv")
...
fig <- ggplot(CO2_BMR, aes(x = Geno.pheno, y = log(Final_MR_CO2_min))) + 
  geom_point() +
  geom_violin() + 
  geom_jitter(fill = "grey", alpha = 0.3) + 
...
write.csv(CO2_BMR, file = "final.analysis.data/Bassiana.finalCO2.sexreversal.analysis.data.clean.csv")
